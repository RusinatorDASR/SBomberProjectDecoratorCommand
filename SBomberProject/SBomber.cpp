    ...
    score(0)
{
	...
	AbstractLevelFactory * factory = new SummerLevelFactory();
...
void CommandExecuter(AbstractCommand* pCommand) { //Добавлена функция
	pCommand->Execute();
	delete pCommand;
}


void SBomber::DeleteDynamicObj(DynamicObject* pObj) //Заменен вызов команды с использованием паттерна Команда
{
	...
	AbstractCommand* DelDynBombComm = new DeleteDynamicObjComm(pObj, vecDynamicObj);
	CommandExecuter(DelDynBombComm);
}

void SBomber::DeleteStaticObj(GameObject* pObj)  //Заменен вызов команды с использованием паттерна Команда
{
	...
	AbstractCommand* DelStaticBombComm = new DeleteStaticObjComm(pObj, vecStaticObj);
	CommandExecuter(DelStaticBombComm);
}
...
vector<DynamicObject*> SBomber::FindAllBombs() const
{
    ...
    for (size_t i = 0; i < vecDynamicObj.size(); i++)
    {
        DynamicObject* pBomb = dynamic_cast<DynamicObject*>(vecDynamicObj[i]);
        if (pBomb != nullptr)
        {
...
void SBomber::DropBomb()  //Заменен вызов команды с использованием паттерна Команда, добавлена новая бомба, вызываемая рандомно, используется паттерн Декоратор
{
    if (bombsNumber > 0)
    {
		//FileLoggerSingleton::getInstance().WriteToLog(string(__FUNCTION__) + " was invoked");
		...
		srand(time(nullptr));
		if (rand() % 2) {
			AbstractCommand* DroppBombCommand = new DroppBombComm(pPlane, vecDynamicObj, bombsNumber);
			CommandExecuter(DroppBombCommand);
		}
		else {
			AbstractCommand* DroppBombCommand = new DroppMegaBombComm(pPlane, vecDynamicObj, bombsNumber);
			CommandExecuter(DroppBombCommand);
		}
